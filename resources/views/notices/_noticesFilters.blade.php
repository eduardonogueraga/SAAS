<div class="my-2 flex sm:flex-row flex-col">
    <div class="flex flex-row mb-1 sm:mb-0">
        <div class="relative">
            <select wire:model="filtroNoticeTipo" id="filtroNoticeTipo"
                    class="appearance-none h-full rounded-l border block appearance-none w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">

                @foreach(trans('data.notices.filters.tipo') as $value => $text)
                    <option value="{{ $value }}"  {{ request('filtroNoticeTipo') === $value ? 'selected' : ''}} >{{ $text }}</option>
                @endforeach
            </select>
        </div>
        <div class="relative">
            <button wire:click="clearFilters({{json_encode(['filtroNoticeTipo'])}})"
