                            ...
                            <li wire:click.stop="openDataModal('{{$d->id}}')" class="shadow-lg pt-4 ml-2 mr-2 rounded-lg">
                                <div  class="block bg-white py-3 border-t pb-4 hover:bg-yellow-100 cursor-pointer">
                                    <div class="px-4 py-2 flex  justify-between">
                                        ...
                                        <span class="ml-3 text-sm font-semibold text-gray-900">ID: {{sprintf("%09d", $d->id)}}
                                              Detección en
                                            @if(isset($d->sensor->terminal->nombre_terminal))
                                                <b> {{$d->sensor->terminal->nombre_terminal}}</b> (Sensor: {{$d->sensor->tipo}})
                                            @else
                                                {{trans('data.sensor.literales.'.$d->sensor->tipo)}}
                                            @endif
                                            @if($d->intrusismo == 1) (<b>Intrusismo</b>)@endif
                                        </span>
                                        </div>
                                        <div class="flex">
                                            <span class="px-4 text-sm font-semibold text-gray-600">Fecha: {{$d->fecha->format('d/m/Y H:i:s')}}</span>
                                    ...
                                    <p class="px-4 py-2 text-sm font-semibold text-gray-700">Sensor: {{trans('data.sensor.literales.'.$d->sensor->tipo)}}  {{$d->sensor->estado}} ({{$d->sensor->valor_sensor}})</p>
                                </div>
                            </li>
                            @break
                        ...
                        @case(3)
                            <li wire:click.stop="openDataModal('{{$d->id}}')" class="shadow-lg pt-4 ml-2 mr-2 rounded-lg">
                                <div class="block bg-white py-3 border-t pb-4 hover:bg-yellow-100 cursor-pointer">
                                    ...
                                    <p class="px-4 py-2 text-sm font-semibold text-gray-700">{{Str::limit(trans('data.logs.literales.'.$d->descripcion),60)}}</p>
                                </div>
                            </li>
                            @break
                                ...
                                <div class="block bg-white py-3 border-t pb-4 hover:bg-yellow-100 cursor-pointer">
                                    <div class="px-4 py-2 flex  justify-between">
                                        <div class="flex">
                                            ...
                                            <span class="ml-3 text-sm font-semibold text-gray-900">ID: {{sprintf("%09d", $d->id)}} @if($d->tipo === 'api') [API] Log de entrada de datos @else [JOB] Log de alarmado @endif</span>
                                        </div>
                                        <div class="flex">
                                            <span class="px-4 text-sm font-semibold text-gray-600">Fecha: {{$d->created_at->format('d/m/Y H:i:s')}}</span>
                                        </div>
                                    </div>
                                    <p class="px-4 py-2 text-sm font-semibold text-gray-700">@if(isset($d->desc)){{Str::limit($d->desc,60)}} @endif @if(isset($d->respuesta_http))| {{Str::limit($d->respuesta_http,60)}} @endif</p>
                                </div>
                            </li>
                            @break
