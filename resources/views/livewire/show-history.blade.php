                ...
                @if($maxCount==0)
                        <li class="flex relative pb-3">
                            ...
                            <div class="flex-shrink-0 w-10 h-10  relative z-10">
                                <div wire:click.stop="openDataModalWithData('{{$h->id}}', '4')" class="flex-shrink-0 w-10 h-10 rounded-full  @if($h->implantado) bg-green-500  @else bg-red-500 @endif inline-flex items-center justify-center text-white relative z-10 cursor-pointer hover:bg-indigo-500">
                        ...
                        <li class="flex relative pb-3">
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="flex flex-wrap">
                                            ...
                                            <div wire:click.stop="openDataModalWithData('{{$h->id}}', '0',{{$i}})" class="h-full flex items-start">
                                                <div class="flex pl-4 pr-4 pt-4  border-2 rounded-lg  border-gray-200 border-opacity-50 bg-orange-400 cursor-pointer hover:bg-yellow-100">
                                                    <h1 class="title-font text-xl font-medium text-gray-900 mb-3">
                                                        Detección en
                                                        @if(isset($h->detections[$i]->sensor->terminal->nombre_terminal))
                                                            <b> {{$h->detections[$i]->sensor->terminal->nombre_terminal}}</b> (Sensor: {{$h->detections[$i]->sensor->tipo}})
                                                        @else
                                                            {{trans('data.sensor.literales.'.$h->detections[$i]->sensor->tipo)}}
                                                        @endif
                                                        @if($h->detections[$i]->intrusismo == 1) (<b>Intrusismo</b>)@endif
                                                    </h1>
                                    ...
                                    @if(isset($h->logs[$i]))
                                        <div class="px-4 my-3 w-3/3">
                                            <div wire:click.stop="openDataModalWithData('{{$h->id}}', '3',{{$i}})" class="h-full flex items-start">
                                                <div class="flex pl-4 pr-4  pt-4 border-2 rounded-lg  border-gray-200 border-opacity-50 bg-cyan-200 cursor-pointer hover:bg-yellow-100">
                                                    <h1 class="title-font text-xl font-medium text-gray-900 mb-3">Log: {{ucfirst(Str::limit(trans('data.logs.literales.'.$h->logs[$i]->descripcion), 20))}} </h1>
